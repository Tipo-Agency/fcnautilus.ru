...
export const sendLeadTo1C = async (data: LeadData): Promise<{ success: boolean; message?: string }> => {
  try {
    ...
    // Настройки Calltouch
    const calltouchSiteId = '48722';
    const calltouchModId = 'ebtz3wdt';
    ...
    calltouchData.append('subject', data.subject || 'Заявка с сайта fcnautilus.ru');
    ...
    if (typeof window !== 'undefined' && (window as any).ym) {
      try {
        (window as any).ym(106321851, 'reachGoal', 'form_submit', {
